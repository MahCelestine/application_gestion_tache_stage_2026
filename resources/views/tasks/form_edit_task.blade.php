<x-layout>
    ...
    <a href="{{ route('tasks.index') }}">dhsq</a>
    <form action="{{ route('tasks.update', $task->id) }}" method="POST">
        ...
        <div>
            ...
            @else
                <p>{{ $task->formatDuration($task->estimated_hours) }} (Calculé via sous-tâches)</p>
                <input type="hidden" name="estimated_h" value="{{ $hours }}">
                <input type="hidden" name="estimated_m" value="{{ $minutes }}">
            @endif
        </div>
        <div>
            <label>Temps réel cumulé : {{ $task->formatDuration($task->actual_hours) }} h</label>
            @if ($task->subtasks->count() === 0)
                <label>Ajouter du temps</label>
                <input type="number" name="add_actual_h" placeholder="0" min="0"> <span>h</span>
                <input type="number" name="add_actual_m" placeholder="0" min="0" max="59"> <span>min</span>
            @else
