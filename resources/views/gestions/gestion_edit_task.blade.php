    ...
    <div class="mx-40" x-data="{ 
        ...
        <form action="{{ route('gestions.task_update', $task->id) }}" method="POST">
            ...
            <div class="border-2 border-gray-300 rounded-xl shadow-md mb-2">
                ...
                <h2 class="text-xl mx-6 my-2">Gestion Facturation Grande tâche : {{ $task->label }}</h2>
        ...
        @if($task->subtasks->isEmpty())
            ...
            <form action="{{ route('gestions.task_reset', $task->id) }}" method="POST" id="reset-task-{{ $task->id }}">
                @csrf
                @method('PATCH')
                <button type="button"
                    onclick="Livewire.dispatch('open-delete-modal', { title: 'Remettre en cours', message: 'Voulez-vous vraiment remettre cette sous-tâche en cours ?', label: 'Remettre en cours', formId: 'reset-task-{{ $task->id }}' })"
