<x-layout>
    ...
    <form action="{{ route('gestions.subtask_update', $subtask->id) }}" method="POST">
        ...
        <div class="flex flex-col">
            ...
            <button type="submit"
                class="bg-blue-500 hover:bg-blue-600 text-white py-4 font-semibold rounded-lg w-[20%] m-auto">Enregistrer les modifications</button>
        </div>
    </form>

    <form action="{{ route('gestions.subtask_reset', $subtask->id) }}" method="POST" onsubmit="return confirm('Remettre cette tâche en cours ? Cela permettra de basculer cette tache dans Accueil.');">
        @csrf
        @method('PATCH')
